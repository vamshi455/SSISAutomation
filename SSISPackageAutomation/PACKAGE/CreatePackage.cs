...
namespace SSISPackageAutomation
{
    class CreatePackage
    {
            ...
            try
            {
                ...
                else if (args[0].ToUpper() == "ALL")
                {
                    foreach (string Group in Groups)
                    {
                        ...
                        pg.GeneratePackage_Test(Group, args[1]);
                    }
                }
                Console.WriteLine("Total Number of packages generated: " + PackageCount);
                return 0;
            }
            catch (Exception ex)
            {
        ...
        public void GeneratePackage_Test(string entity, string filepath)
        {
            try
            {
